...
import {
  ...
  clientPortalSecondaryLinkButtonClassName,
    ...
    <div className="space-y-8">
      {meError && (
        <p className="rounded-md border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
          ...
          <Link href="/client-portal/request-link" className={`${clientPortalLinkClassName} underline`}>
        ...
        <div className="rounded-lg border bg-white p-6 shadow-sm">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <Link
              ...
              className={`inline-flex items-center rounded-md px-4 py-2 text-sm font-semibold ${clientPortalPrimaryButtonClassName}`}
      ...
      <div className="rounded-lg border bg-white shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-6 py-4">
          ...
          <Link href="/client-portal/invoices" className={`text-sm ${clientPortalLinkClassName}`}>
            See all
          </Link>
        </div>
          ...
          {(invoicesQuery.data ?? []).slice(0, 8).map((inv) => {
            ...
            return (
              <li key={inv.id}>
                ...
                >
                  <span className="text-left font-medium text-slate-900">{inv.invoiceNumber}</span>
                  <span className="text-left text-slate-600">${toDollars(inv.total)}</span>
