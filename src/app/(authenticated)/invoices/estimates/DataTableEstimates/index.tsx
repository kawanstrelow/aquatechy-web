...
interface DataTableProps<TValue> {
  columns: ColumnDef<EstimateListRow, TValue>[];
  data: EstimateListRow[];
  clients?: { id: string; name: string }[];
  companies?: Array<{ id: string; name: string }>;
  onCompanyChange?: (companyId: string) => void;
  onRowClick?: (row: EstimateListRow) => void;
}

export function DataTableEstimates<TValue>({
  columns,
  data,
  clients = [],
  companies = [],
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, _columnId, filterValue) => {
      ...
      const amount = typeof estimate.amount === 'number' ? estimate.amount : 0;
  ...
  const statusOptions = [
    ...
    { key: 'accepted', value: 'accepted', name: 'Accepted' },
    { key: 'declined', value: 'declined', name: 'Declined' },
    { key: 'expired', value: 'expired', name: 'Expired' },
